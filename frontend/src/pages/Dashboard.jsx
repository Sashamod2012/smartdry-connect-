...
const statusStyle = {
  RUNNING: "bg-emerald-500/15 text-emerald-600 border-emerald-500/40",
  PROCESSING: "bg-emerald-500/15 text-emerald-600 border-emerald-500/40",
  IDLE: "bg-slate-500/15 text-slate-600 border-slate-500/40",
  PLANNED: "bg-slate-500/15 text-slate-600 border-slate-500/40",
  PAUSED: "bg-amber-500/15 text-amber-600 border-amber-500/40",
  COMPLETE: "bg-sky-500/15 text-sky-600 border-sky-500/40",
  COMPLETED: "bg-sky-500/15 text-sky-600 border-sky-500/40",
};
  ...
  return (
    <div className="space-y-6 fade-up" data-testid="dashboard-page">
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-4">
        <div>
          <h1 className="text-3xl lg:text-4xl font-extrabold tracking-tight text-slate-900">Operations Dashboard</h1>
        ...
        <div className="flex flex-wrap gap-2">
          ...
          <Link to="/new-batch" data-testid="dash-goto-newbatch-btn" className="flex items-center gap-2 px-4 py-2 rounded-lg border border-slate-300 hover:border-emerald-500/50 hover:text-emerald-600 text-slate-600 text-sm font-semibold transition-colors">
      ...
      <div className="sd-card px-5 py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <div className="w-full sm:w-64">
          <div className="flex justify-between text-[10px] font-mono text-slate-500 mb-1.5">
            ...
            <span className="text-emerald-600 font-bold">{t?.progress_pct ?? 0}%</span>
          </div>
          <div className="h-2 rounded-full bg-[#f1f5f9] border border-slate-200 overflow-hidden">
      ...
      <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
        <div className="sd-card p-5" data-testid="active-batch-card">
          ...
          <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 gap-3">
            <div>
              ...
              <div className="font-mono text-sm text-slate-700 mt-0.5">{t?.active_batch?.tray_quantity || 24} / 24</div>
            </div>
            <div>
              <div className="text-[9px] font-mono uppercase tracking-widest text-slate-500">Operator</div>
              <div className="text-sm text-slate-700 mt-0.5 truncate">{t?.active_batch?.operator || "C. Eze"}</div>
            </div>
          </div>
        </div>
        ...
        <div className="sd-card p-5" data-testid="process-monitoring-card">
          ...
          <div className="mt-4 pt-4 border-t border-slate-200 grid grid-cols-2 gap-3">
            <div className="flex items-center justify-between rounded-lg bg-[#f1f5f9] border border-slate-200 px-3 py-2">
              <span className="flex items-center gap-1.5 text-[10px] font-mono text-slate-500"><Droplets className="w-3 h-3 text-sky-600" /> HUMIDITY</span>
              <span className="font-mono text-[11px] font-bold text-sky-600 whitespace-nowrap" data-testid="stat-humidity">{t ? `${t.humidity_pct} %RH` : "—"}</span>
            </div>
            <div className="flex items-center justify-between rounded-lg bg-[#f1f5f9] border border-slate-200 px-3 py-2">
              <span className="flex items-center gap-1.5 text-[10px] font-mono text-slate-500"><Flame className="w-3 h-3 text-emerald-600" /> GAS</span>
              <span className="font-mono text-[11px] font-bold text-emerald-600 whitespace-nowrap" data-testid="stat-gas">{t ? `${t.gas_ppm} PPM · ${t.gas_status}` : "—"}</span>
            </div>
          </div>
        </div>
        ...
        <div className="sd-card p-5 sm:col-span-2 lg:col-span-1" data-testid="energy-config-card">
          ...
          <div className="space-y-2">
            {[
              { icon: Flame, label: "Thermal Source", value: t?.power.thermal_source || "LPG Hot-Air", tone: "text-amber-600" },
              { icon: Zap, label: "Electrical Source", value: t?.power.electrical_source || "Solar PV + Battery", tone: "text-sky-600" },
              { icon: PlugZap, label: "Grid", value: t?.power.grid_status || "OFF-GRID — NOT REQUIRED", tone: "text-slate-500" },
              { icon: Sun, label: "Solar", value: t ? `${t.power.solar_status} · ${t.power.solar_w} W` : "—", tone: "text-sky-600" },
              { icon: BatteryCharging, label: "Battery", value: t ? `${t.power.battery_status} · ${t.power.battery_pct}%` : "—", tone: "text-emerald-600" },
            ].map((r) => (
              <div key={r.label} className="flex items-center justify-between rounded-lg bg-[#f1f5f9] border border-slate-200 px-3 py-2">
      ...
      <div className="grid lg:grid-cols-3 gap-4">
        <div className="sd-card p-5 lg:col-span-2" data-testid="connectivity-flow-widget">
          ...
          <div className="flex flex-col sm:flex-row items-stretch sm:items-center gap-3">
            {[
              { icon: Cpu, title: "ESP32 + Sensors", sub: "Temp/RH · Load cell · Gas · OLED", color: "text-emerald-600 border-emerald-500/40 bg-emerald-600/10" },
              { icon: Cloud, title: "SmartDry Connect Cloud", sub: "IoT ingestion · Batch records", color: "text-sky-600 border-sky-500/40 bg-sky-600/10" },
              { icon: MonitorSmartphone, title: "Dashboard + QR Traceability", sub: "Monitoring · Analytics · Traceability", color: "text-emerald-600 border-emerald-500/40 bg-emerald-600/10" },
            ].map((n, i) => (
              <div key={n.title} className="flex items-center gap-3 flex-1">
                <div className={`flex-1 rounded-xl border p-4 ${n.color.split(" ").slice(1).join(" ")}`}>
                  ...
                  <div className="mt-2 text-sm font-semibold text-slate-800">{n.title}</div>
          ...
          <div className="mt-4 grid grid-cols-3 gap-3 text-center">
            <div className="rounded-lg bg-[#f1f5f9] border border-slate-200 py-2.5">
              <div className="font-mono text-lg font-bold text-sky-600">{t?.connectivity.latency_ms ?? "—"}<span className="text-[10px] text-slate-500"> ms</span></div>
              <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider">Latency</div>
            </div>
            <div className="rounded-lg bg-[#f1f5f9] border border-slate-200 py-2.5">
              <div className="font-mono text-lg font-bold text-sky-600">{t?.connectivity.rssi_dbm ?? "—"}<span className="text-[10px] text-slate-500"> dBm</span></div>
              <div className="text-[10px] font-mono text-slate-500 uppercase tracking-wider">Signal</div>
            </div>
            <div className="rounded-lg bg-[#f1f5f9] border border-slate-200 py-2.5">
              <div className="font-mono text-lg font-bold text-emerald-600">{t?.connectivity.packet_hz ?? "—"}<span className="text-[10px] text-slate-500"> Hz</span></div>
        ...
        <div className="sd-card p-5" data-testid="power-status-card">
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3 rounded-lg bg-[#f1f5f9] border border-slate-200 p-3">
              <Flame className="w-5 h-5 text-amber-600 mt-0.5" />
              <div>
                <div className="text-sm font-semibold text-slate-800">LPG Thermal Source</div>
                <div className="text-[11px] text-slate-500">Primary thermal source for hot-air drying</div>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg bg-[#f1f5f9] border border-slate-200 p-3">
              <Sun className="w-5 h-5 text-sky-600 mt-0.5" />
              <div>
                <div className="text-sm font-semibold text-slate-800">Solar PV Array</div>
                <div className="text-[11px] text-slate-500">Electrical power for controls, sensors, ESP32 and communications</div>
              </div>
            </div>
            <div className="flex items-start gap-3 rounded-lg bg-[#f1f5f9] border border-slate-200 p-3">
              <BatteryCharging className="w-5 h-5 text-emerald-600 mt-0.5" />
              <div>
                <div className="text-sm font-semibold text-slate-800">Battery Buffer</div>
